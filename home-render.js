    ...
    if (sv && data.services) {
      sv.innerHTML = data.services.map(s => `
        <article class="glass rounded-2xl p-6 card">
          ...
          ${s.cta ? `<a href="${s.cta}" class="mt-4 inline-block px-4 py-2 rounded-xl bg-brand text-neutral-900 font-semibold text-sm">Get Started</a>` : ''}
        </article>
      `).join('');
    }
    ...
    if (pk && data.packages) {
      pk.innerHTML = data.packages.map(p => `
        <article class="glass rounded-2xl p-6 card">
          ...
          <a href="${p.cta}" class="mt-4 inline-block px-4 py-2 rounded-xl bg-brand text-neutral-900 font-semibold text-sm">Request Quote</a>
        </article>
      `).join('');
    }
  } catch(e) {}
